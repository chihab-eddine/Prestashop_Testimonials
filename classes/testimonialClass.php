...
class TestimonialClass extends ObjectModel
{
    ...
    public static function getTestimonialByStatus()
    {
        $query = new DbQuery();
        $query->select('ts.id_costumer, ts.position, ts.title , ts.message , ts.file , cm.firstname , cm.lastname');
        $query->from('testimonial', 'ts');
        $query->innerJoin('customer', 'cm', 'ts.id_costumer = cm.id_customer');
    ...
    {
        $query = new DbQuery();
        $query->select('ts.`id_testimonial`, ts.`position`');
        $query->from('testimonial', 'ts');
        $query->orderBy('ts.`position` ASC');
        ...
        foreach ($tabs as $tab) {
            if ((int) $tab['id_testimonial'] == (int) $this->id) {
                $moved_tab = $tab;
            }
        }
        ...
        return (Db::getInstance()->execute('
            ...
            WHERE `id_testimonial` = ' . (int)$moved_tab['id_testimonial']));
    }
}
